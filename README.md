...
- **可玩角色**：多名角色可选，开始菜单即可预览、旋转查看。默认 **FOX** 是一个完整绑定的狐狸滑雪角色（Humanoid 骨骼 + 双腿 IK + 尾巴骨骼 + 独立雪板骨骼 + 5 段骨骼动画）。
...
- **游玩模式**：标准 / 计时挑战 / 一命通关 / **无尽模式**，主菜单可切换。
- **随机赛道**：赛道可选「标准 / 随机」，随机档每局生成不同的地形与物件布局。
- **收集道具**：赛道上可拾取 6 种道具——**加速 / 加分 / 护盾 / 磁铁 / 滞空 / 无敌**，各有效果与时限、拾取特效，HUD 显示剩余时间；最高速度可突破 130 km/h。
- **沉浸细节**：滑过之处留下**雪痕**；第一人称可见雪板前端与手套；摔车有翻滚动画，并按「人物左侧 + 成绩卡右侧」呈现结果菜单；抵达终点有人物庆祝 + **烟花** + 成绩卡，可**拖动鼠标选择庆祝视角**，并会**停止滑行音效**。
- **开始游戏**：点击「开始游戏」立即出发；角色 / 模式 / 赛道分别在各自菜单页选择。
- **每日挑战**：每天轮换一个目标（得分 / 旗门 / 特技 / 速度 / 用时 / 滞空），达成有提示。
- **拍照模式**：按 `K` 隐藏界面、自由环绕缩放并保存截图。
- **双视角**：第三人称（默认，镜头拉近、动作清晰）与第一人称（按 `V` 切换），设置中可切换并记忆。
- **下坡速度感**：镜头沿坡面向下俯视，配合更陡的坡度，强化向下冲刺的刺激感。
